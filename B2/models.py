...
# Models.py
# Modified for adapting tensorflow backend April 2019 by QL
...
class DenoisingWavenet():
    ...
    def setup_model(self, load_checkpoint=None, print_model_summary=False):
        ...
        if os.path.exists(self.checkpoints_path) and util.dir_contains_files(self.checkpoints_path):
            ...
            os.makedirs(self.config['training']['path'], exist_ok=True)
    ...
    def fit_model(self, train_set_generator, num_train_samples, test_set_generator, num_test_samples, num_epochs):
        ...
        self.model.fit_generator(train_set_generator,
                                 steps_per_epoch = int(num_train_samples/self.config['training']['batch_size']),
                                 nb_epoch=num_epochs,
                                 validation_data=test_set_generator,
                                 validation_steps=int(num_test_samples/self.config['training']['batch_size']),
    ...
    def build_model(self):
        ...
        # input_layer = keras.layers.Input(shape=(400, 1))
        # conv1 = keras.layers.Conv1D(filters=32,
        #                kernel_size=8,
        #                strides=1,
        #                activation='relu')(input_layer)
        #
        # data_out = keras.layers.Conv1D(filters=self.config['model']['filters']['depths']['res'],
        #                                kernel_size=(self.config['model']['filters']['lengths']['res'],),
        #                                border_mode='same',
        #                                bias=False,
        #                                name='initial_causal_conv')(input_layer)
        ...
        # data_expanded = keras.backend.expand_dims(data_input, -1)
        ...
        data_out = keras.layers.Conv1D(filters = self.config['model']['filters']['depths']['res'],
                                       kernel_size = (self.config['model']['filters']['lengths']['res'],), padding='same',
                                       use_bias=False,
        ...
        return keras.models.Model(input=[data_input, condition_input], output=[data_out_speech, data_out_noise])
...
if __name__ == "__main__":
    ...
    MC = DenoisingWavenet(config=config_file)
    # denoiseModel = MC.setup_model()

    print(MC.model.summary())
    from keras.utils import plot_model
    plot_model(MC.model, to_file='Model.png')

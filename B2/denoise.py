...
def denoise_sample(model, input, condition_input, batch_size, output_filename_prefix, sample_rate, output_path):
    ...
    regain_factor = 1
    # regain_factor = 0.06 / util.rms(input['noisy'])
    ...
    for batch_i in tqdm.tqdm(range(0, num_batches)):
        ...
        denoised_output_fragments = model.denoise_batch({'data_input': input_batch * regain_factor, 'condition_input': condition_batch})
